...
export const WalletGenerator = ({
  ...
  async function handleRefresh() {
    ...
    const newWallets = wallets.map(async (wallet) => {
      const response = await axios.post(
        isSolana
          ? isMainnet
            ? (process.env.NEXT_PUBLIC_SOLANA_MAIN_NET_URL as string)
            : (process.env.NEXT_PUBLIC_SOLANA_DEV_NET_URL as string)
          : (process.env.NEXT_PUBLIC_ETHEREUM_MAIN_NET_URL as string),
        isSolana
          ? {
              jsonrpc: "2.0",
              id: 1,
      ...
      const newbalance = isSolana
        ? response.data.result.value / Math.pow(10, 9)
        : parseInt(response.data.result, 16) / Math.pow(10, 18);
  ...
  async function handleAddWallet() {
    ...
    if (isSolana) {
      ...
      setWallets((prevWallets) => {
        const newWallets = [
          ...prevWallets,
          {
            ...
            publicKey,
            id:
              prevWallets.length > 0
                ? prevWallets[prevWallets.length - 1].id + 1
                : 0,
            balance,
          },
        ];
      ...
      const response = await axios.post(
        isMainnet
          ? (process.env.NEXT_PUBLIC_ETHEREUM_MAIN_NET_URL as string)
          : (process.env.NEXT_PUBLIC_ETHEREUM_DEV_NET_URL as string),
        {
          jsonrpc: "2.0",
          id: 1,
          method: "eth_getBalance",
  ...
  return (
    <div className="">
      {clearWallet ? (
        <div className="fixed inset-0 z-50 backdrop-blur-sm flex flex-col justify-center items-center text-white w-screen">
          <div className="flex flex-col md:w-[30%] bg-gray-950 border border-gray-700 rounded-lg w-[80%] ">
            ...
            <div className="text-gray-600 mt-3 ml-3">
              This action cant be undone.This will permanently delete your
              Wallets and Key from the localstorage.
            </div>
            <div className="flex justify-end mt-12 mb-4">
              <button
                className="bg-gray-200 rounded-md w-20 h-9 text-black mr-3 transform transition-transform duration-300 hover:scale-105"
                onClick={() => {
              ...
              <button
                className="bg-red-900 rounded-md w-20 h-9 mr-3 transform transition-transform duration-300 hover:scale-105"
                onClick={() => {
                  setWallets([]);
                  // setWalletCount(1)
      ...
      <div className="flex justify-center mt-16 ">
        <div className="flex md:flex-row flex-col justify-between md:w-[64%] w-[96%]  ">
          ...
          <div className="flex md:flex-row justify-around md:justify-center mt-10 md:mt-0 md:ml-0  flex-wrap gap-[14px]">
            <button
              className="mt-1 bg-white text-black hover:bg-gray-800 hover:text-gray-200 rounded-lg  w-24 h-10 md:w-24 md:h-12 transform transition-transform duration-300 hover:scale-105"
              onClick={handleRefresh}
            >
              Refresh
            </button>
            <button
              className="mt-1 bg-white text-black hover:bg-gray-800 hover:text-gray-200  h-10 w-24 md:h-12 md:w-24 mr-2 md:mr-0 rounded-lg transform transition-transform duration-300 hover:scale-105"
              onClick={handleAddWallet}
            >
              Add Wallet
            </button>
            <button
              className="mt-1 bg-white text-black hover:bg-gray-800 hover:text-gray-200  h-10 w-32 md:h-12 md:w-28  rounded-lg transform transition-transform duration-300 hover:scale-105"
              onClick={() => {
                setClearWallet(true);
              }}
            >
